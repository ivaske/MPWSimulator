...
public class ListViewCellDemo extends Application {
	...
	private static class TemperatureCell extends ListCell<Double> {
		...
		protected void updateItem(Double item, boolean empty) {
			...
			if (item != null & !empty) {
				...
				textProperty().bind(Bindings.concat(slider.valueProperty(), "°C"));
				textFillProperty().bind(new ObjectBinding<Paint>() {
					...
					protected Paint computeValue() {
						return slider.getValue() < 0 ? Color.BLUE : slider.getValue() > 21 ? Color.RED : Color.BLACK;
					}
				});
				if (!isEditing()) {
					circle.setFill(item < 0 ? Color.BLUE : item > 21 ? Color.RED : Color.BLACK);
					setGraphic(circle);
				}
			} else {
				textProperty().unbind();
